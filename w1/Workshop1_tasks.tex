...
\pagestyle{fancy}
\fancyhf{}
\lhead{\textbf{CMP1138: Foundations of Programming}}
\rhead{\textbf{Week 13}}
...
\begin{center}
    \LARGE \textbf{Lab Sheet 13: Version Control with Git \& GitHub} \\
    \vspace{0.5cm}
...
\begin{itemize}
    ...
    \item Understand how to \textbf{Ignore} C\# specific build files.
...
git init
    ...
    \begin{itemize}
        \item Create a file named \texttt{hello.txt} and add some text to it.
...
\begin{itemize}
    ...
    \item Create an account using your \textbf{University Email Address} (recommended for academic benefits) or use your existing personal account if you prefer.
...
\begin{enumerate}
    ...
    \begin{itemize}
        \item Name: \texttt{CMP1138-Week13}
        \item Visibility: \textbf{Public} (or Private).
...
\begin{enumerate}
    ...
    \item \textbf{Add Code:} Create a new file \texttt{Program.cs} with a basic "Hello World" in C\#.
    \item \textbf{Check Status:} Run \texttt{git status}.
    \item \textbf{Stage All:} Run \texttt{git add .} to stage both changes.
    \item \textbf{Commit:} Run \texttt{git commit -m "Added C\# program and updated text"}.
...
\section*{Task 5: Assessment 3 Backup (Using .gitignore)}
...
\section*{Task 6: Forking a Repository \& Pull Requests}
