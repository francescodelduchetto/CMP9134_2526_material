...
\usepackage{hyperref}
\usepackage{tikz}
\usetikzlibrary{shapes, arrows, positioning, shadows, calc, decorations.pathreplacing}
...
\begin{frame}[fragile]
    ...
    \begin{tikzpicture}[scale=0.6, transform shape]
        ...
        \node[align=center] at (-3, -3) {\textbf{Waterfall}\\Single pass\\Sequential};
        \node[align=center] at (3, -3) {\textbf{Staged Delivery}\\(Prince2)\\Building block by block};

        % Graphics (Simulated)
        \draw[red, thick, ->] (-3, 1.5) circle (0.5);
        \foreach \x in {2, 2.5, 3} \draw[blue, thick] (\x, 1.5) rectangle (\x+0.3, 1.8);
        \draw[black, thick, ->] (-4, -2) -- (-3, -2.5) -- (-2, -3);
        \foreach \x in {2, 2.5, 3} \draw[black, thick] (\x, -2) rectangle (\x+0.3, -1.7);
